...
→ **S111 task**: Build `/network` page with full-bleed SVG container, zoom/pan controls, and
   "depth" slider (1–3 hops) using the existing `socialGraphService.getTrustGraphAggregate()` call
   (graph fetches live on `socialGraphService` in `api.ts`, not `socialGraphClient`).
...
- **Capped growth**: click-to-expand fetches only the clicked node's 1-hop neighborhood, not the
  full graph. A new endpoint `GET /trust/neighborhood/:userId` (S111 backend task, added to
  `trustGraph.ts` alongside `/trust/graph`) returns the 1-hop set.
...
→ **S111 tasks**:
  1. Add `GET /trust/neighborhood/:userId` to social-graph-service `trustGraph.ts` (S111 backend item)
...
"You're connected to N people across M communities" — a compound stat that captures both the graph
dimension (people) and the membership dimension (communities). Both numbers are available from
**existing read APIs**, not a raw DB query: N = the ego-graph node count the profile section already
fetches (`socialGraphService.getTrustGraphAggregate()`); M = `communityService.getMyCommunities()`
length. (Note: a literal "people you helped" count is **not** derivable from
`reputation.karma_records` — that table's `user_id` is the *recipient* and there is no
`karma_awarded_to` column. The trust-graph connection count is the honest, available stand-in, since
trust edges form from helping. A true "helped" aggregate would be a new reputation endpoint, out of
S111 scope.)

→ **Surface**: Profile page, possibly Dashboard

→ **S111 task**: Add a `BelongingPulse` component fed by the already-loaded ego graph
   (`graphData.nodes.length - 1`) + `communityService.getMyCommunities(userId)` length, rendered as
   a stat line above the profile graph.
