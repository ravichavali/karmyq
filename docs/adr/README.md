...
- [ADR-059: Dependency Vulnerability Remediation + Blocking CI Security Gate](ADR-059-dependency-security-gate.md) — **Implemented** (Sprints 123–125: shared registry and renewal cadence; Sprint 128 amendment, deployment pending: reject unavailable audit evidence, remove image-size through SDK remediation, and patch release-time web/XML/YAML findings)
...
- [ADR-082: Reputation Disclosure Boundary](ADR-082-reputation-disclosure-boundary.md) — **Accepted** (amended Sprint 129: a denied community-trust aggregate is `200 { data: null }`, identical across causes)
