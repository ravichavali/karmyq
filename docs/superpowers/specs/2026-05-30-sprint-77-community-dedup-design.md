...
9. **This migration runs on the demo server**, against `karmyq_prod`. It is a one-time data repair — the **executing agent** SSHes in and runs it (backup → dry-run → verify → real run), not `deploy.sh` auto-migrations. `git push` ships the code fixes; the data repair is a deliberate, agent-run step. **Verified access (2026-05-31):** `ssh ubuntu@karmyq.com`, container `karmyq-postgres`, **user/DB `karmyq_prod`** (not `karmyq_user`); baseline confirmed at 707 total / 707 active / 23 distinct identities; backup procedure tested (`~/backups/karmyq_prod-20260531-044657.dump`, 7.4M).
