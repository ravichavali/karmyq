...
describe('apps/mobile stays aligned to its Expo SDK', () => {
  ...
  it('every expo-family package shares the SDK major', () => {
    ...
    const misaligned = Object.entries(allDeps)
      ...
      // Guard as the lockfile assertion does: a workspace:/file: protocol range
      // would make semver.major(null) throw a raw error instead of failing cleanly.
      .filter(([, range]) => semver.validRange(range))
