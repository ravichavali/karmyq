...
- Prove the lockfile with strict `npx -y npm@11.19.0 ci`, which must leave `package-lock.json` byte-identical, and with `npm ls --all`, which must report no `invalid`, `missing` or `ERR!` entries.
...
4. **The peer ranges of a moved package are unsatisfied**, which the probe does not check. Expected behavior: `npm ls --all` has no `invalid` line and the step fails otherwise. (Task 1 Step 6.)
