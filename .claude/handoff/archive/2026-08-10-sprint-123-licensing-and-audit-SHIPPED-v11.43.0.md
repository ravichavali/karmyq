...
> # ✅ SHIPPED AND DEPLOYED — v11.43.0. ARCHIVED 2026-08-10.
>
> **PR #198 squash-merged as `9dd080afe6c4eecd805c5c8bd30dd9912c1594c5`.** CI/CD, integration,
> security, CodeQL, image builds, deployment and health checks all green. `Docker test` hit a
> transient npm `ECONNRESET` and passed on failed-job retry.
>
> | Definition of Done | Result |
> |---|---|
> | **GitHub license detection** (the externally-visible proof, and why `LICENSE` had to stay byte-exact) | **GNU AGPLv3** — `license.spdx_id` is `AGPL-3.0`, no longer `null` |
> | `karmyq.com` login smoke | **400 `VALIDATION_ERROR`** and **401 `UNAUTHORIZED`**, as expected |
> | `karmyq.org` landing, AGPL concept, ADR-092, ADR-093 | all **200** |
> | Master manifest | **11.43.0** |
>
> ⚠️ **Everything below this banner was written BEFORE the merge and still says "awaiting merge".**
> It is preserved as the working record; the banner is the truth. The stale text was deliberately
> **not** corrected on `master` — a docs-only master push triggers a full deploy → demo 502s
> ([[feedback_no_docs_push_to_master]]) — so the reconciliation happened on the Sprint 124
> planning branch instead. That is the correct handling of
> [[feedback_handoff_stranded_by_merge]], not an oversight.
>
> **Carried forward → see `CURRENT_HANDOFF.md`.** The load-bearing one: the `image-size` audit
> exemption **expires 2026-08-17** and will fail the build that day.
