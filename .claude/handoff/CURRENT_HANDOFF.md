...
> 🚧 **D4 IN PROGRESS (2026-09-23)** — implemented, and every SDLC gate passed; next is the PR, CI and merge authorization. Focused plan:
...
| **D4** | **expo-server-sdk 6.1.0 → 7.2.0 (#230)** | **IMPLEMENTED, ALL GATES PASSED 2026-09-23 — next: push, PR, CI (plan Task 4 Steps 5–7), then merge authorization.** Gates: **full uncached suite** 27/27 (5m58s) and drift gate 41/41.<br>**`/simplify`** (4 agents): efficiency found nothing. Applied: ts-node as the child's loader (probed first: its emit follows the tsconfig exactly as the tsc API's does); `execFile` + `timeout` in place of the hand-rolled deadline; small cleanups; and the **named `Expo` export** in `expoPush.ts` instead of `mod.default` (`.default` is the class only while Node marks a required ES module `__esModule`; probed, runtime-identical today; this supersedes "the default export" in the surface list below). Skipped: a synchronous stdin read (unverified on Linux). Follow-ups (load the SDK at boot; type it) → `docs/IDEAS.md` [2026-09-23]. All six injections re-proven on the simplified harness (5, 1, 2, 1, 1, 1).<br>**`/code-review`** medium: 0 code bugs, 1 doc contradiction fixed.<br>**`/security-review`**: no vulnerabilities (registry.npmjs.org, matching integrity, 1,844 lockfile entries before and after, no install scripts).<br>**Landing pages** regenerated: notification content committed with D4, three stale unrelated pages (cleanup, geocoding, reputation) in their own labeled commit, timestamp/sha churn reverted.<br>Earlier status: **IMPLEMENTED on this branch 2026-09-23.** Commits: `b7bc1b11` (the real-SDK test and the corrected comment), `42a5862a` (Dependabot #230 cherry-picked unchanged), then the docs/version commit (CONTEXT.md's D4 section and push-token corrections, v11.66.0). Evidence: the new test passed **5/5 on 6.1.0** and, unchanged, **5/5 on 7.2.0**. Injections I1–I6 failed **5, 1, 2, 1, 1, 1**, as planned; I6 showed a stalled push API gets the child killed at the deadline and Jest exits cleanly. Strict `npm ci` exited 0 with the lockfile untouched, `npm ls` problems were unchanged, the declarations gate passed 10/10 and the service suite 78/78. The focused plan was written, reviewed and approved 2026-09-23 (native execution); the branch was cut fresh from the deployed `8fbbeb5f`. Plan: `docs/superpowers/plans/2026-09-23-sprint-131-pr-d4-expo-server-sdk.md`. BUG-049 and BUG-051 have both shipped. Re-check #230 against the new master (it auto-rebases) and verify v7 behavior against the installed `node_modules`, never the changelog. **Importer inventory, verified 2026-09-23 by `git grep` over tracked source:** exactly one importer, `services/notification-service/src/lib/expoPush.ts:9` (**written** as a dynamic `import()`, but **corrected 2026-09-23:** the service compiles `"module": "commonjs"`, so the build emits `require('expo-server-sdk')` — `dist/lib/expoPush.js:46` — which loads a pure-ESM package only because Node can `require()` an ES module), and exactly one declarer, `services/notification-service/package.json`. ⚠️ **No existing test exercises the SDK:** the only test touching `expoPush` is BUG-051's `sprint-131-push-internal-auth.test.ts`, which **mocks it out entirely** — so a green suite says nothing about v7. D4 needs its own real-module test of the exact surface `expoPush.ts` uses, verified 2026-09-23: **the default export** (`mod.default`, line 10 — the likeliest thing a major breaks, since v6 went pure-ESM), a **no-arg constructor**, static `Expo.isExpoPushToken`, `chunkPushNotifications`, and `sendPushNotificationsAsync`, whose tickets are read for `status === 'error'`, `message` and `details` |
...
4. Order decided 2026-09-22: **(1) BUG-049 — SHIPPED v11.64.0; (2) BUG-051 — SHIPPED v11.65.0; (3) D4 (expo-server-sdk #230) — IMPLEMENTED and all gates passed 2026-09-23; resume at plan Task 4 Step 5 (push, PR, CI).** Scope: the D4 row above and the plan. Then D5–D7 (node-fetch, zod, next), one major per PR.
...
**Next unchecked action: D4 plan Task 4 Steps 5–7**
(`docs/superpowers/plans/2026-09-23-sprint-131-pr-d4-expo-server-sdk.md`): push, open the PR, verify CI
from the logs, record the PR number and CI evidence here, then ask the maintainer for merge authorization.
The suite, the drift gate and all three SDLC gates have passed; see the D4 row. Tasks 1–3 are done and committed on this branch; the ledger is
