...
4. Order decided 2026-09-22: **(1) BUG-049 — SHIPPED v11.64.0; (2) BUG-051 — its own PR, IN PROGRESS, HIGH severity; (3) D4 (expo-server-sdk #230).** Do not skip BUG-051: it is an unauthenticated public endpoint. Scope for each: the BUG-051 block below, the BUG-049 notes below, and the D4 row above.
...
**Next unchecked action: BUG-051** — `POST /api/notifications/push/send` is unauthenticated (see the block below). Branch `agent/claude/sprint-131-bug-051-push-auth` is cut from the deployed `96ffa619`. ⚠️ **First step is a read-only demo check that `~/karmyq/.env.demo` defines `INTERNAL_SECRET`** — a demo operation needing its own authorization. Failing the middleware closed without that secret present would turn the endpoint into a 503. After BUG-051 ships: D4 (expo-server-sdk, #230).
