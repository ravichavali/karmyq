...
The fix is two layers: `ipKeyGenerator(req.ip)` in the shared key generator, and `app.set('trust proxy', 1)` in the nine limiter-mounting services. `1` is the only safe value — `true` lets a client spoof `req.ip`, `'loopback'` does not match the Docker gateway. Because nginx uses `$proxy_add_x_forwarded_for` (real client appended **last**), one trusted hop is spoof-proof; probed against the installed express 5.2.1 / proxy-addr 2.0.7 with forged chains of one, two and four entries. Gated by `tests/regression/sprint-131-rate-limit-trust-proxy.test.ts` (registry-derived, AST over tracked `.ts` **and** `.js`, 5 injection proofs) and `packages/shared/src/middleware/__tests__/sprint-131-rate-limit-key.test.ts` (behavioural). ⚠️ Limits are **per-IP everywhere** now, not per-user; making per-user keying live needs a mount-order redesign, logged in `docs/IDEAS.md` [2026-09-22].
