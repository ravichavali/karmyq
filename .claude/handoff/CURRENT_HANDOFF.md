...
2. BUG-049 work is on `agent/claude/sprint-131-bug-049-rate-limit-key` (already cut from `e5f7d8e1`). D4 cuts `agent/claude/sprint-131-d4-expo-server-sdk` fresh from the newly deployed `origin/master` after BUG-049 merges.
...
4. Order decided 2026-09-22: **BUG-049 first** (built, green, gates outstanding), **then D4**. Scope: the BUG-049 notes below and the D4 row above.
...
**Next unchecked action: run the three SDLC gates on the BUG-049 branch diff** (`/simplify`, `/code-review high`, `/security-review`), then push, open the PR, and ask for merge authorization. The full suite is already green (exit 0, 27/27 Turbo tasks) and the drift gate passes 41/41. **After BUG-049 merges and deploys:** D4 (expo-server-sdk, #230) from a fresh branch off the newly deployed `origin/master`.
...
- **Not auth-only.** `globalRateLimiter` is mounted app-level in **seven** services; `cleanup-service` builds its own. Eight services were affected.
- **The `user:<userId>` branch was unreachable repo-wide** — every limiter sits ahead of `authMiddleware`. So `rateLimiters.standard` gave request-service's ~12 route groups **one** 60/min bucket for the entire user base: enabling limiting would have throttled the whole site, not just risked an `/auth/*` lockout. That is the likeliest reason `RATE_LIMIT_DISABLED=true` was set.
...
The fix is two layers: `ipKeyGenerator(req.ip)` in the shared key generator, and `app.set('trust proxy', 1)` in the eight limiter-mounting services. `1` is the only safe value — `true` lets a client spoof `req.ip`, `'loopback'` does not match the Docker gateway. Because nginx uses `$proxy_add_x_forwarded_for` (real client appended **last**), one trusted hop is spoof-proof; probed against the installed express 5.2.1 / proxy-addr 2.0.7 with forged chains of one, two and four entries. Gated by `tests/regression/sprint-131-rate-limit-trust-proxy.test.ts` (registry-derived, AST, 4 injection proofs) and `packages/shared/src/middleware/__tests__/sprint-131-rate-limit-key.test.ts` (behavioural). ⚠️ Limits are **per-IP everywhere** now, not per-user; making per-user keying live needs a mount-order redesign, logged in `docs/IDEAS.md` [2026-09-22].
