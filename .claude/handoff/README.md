...
Work through this before the second checkout starts any sprint:

- [ ] **Host verified**: run `uname -s` on each machine and apply only that host's gotchas
      (`MINGW64_NT-*` = Windows, `Darwin` = Mac). They do not transfer.
- [ ] **Bootstrap verified**: `npm ci` then `npm run hooks:install` on the new clone. `.npmrc` sets
      `ignore-scripts=true`, so hooks are NOT installed automatically.
- [ ] **Hooks proven live**: a push that finishes silently and instantly means no hook ran. Git
      reads only `core.hooksPath` when it is set — confirm it points at the installed hooks.
- [ ] **Memory is not portable**: `MEMORY.md` and its files live outside the repo in
      `~/.claude/projects/<project>/memory/`. Sync that directory; a fresh clone has none of it.
- [ ] **File ownership explicit**: split sprints on file-disjoint boundaries, by service. Record
      each lane's owned paths in its lane file.
- [ ] **No shared-package split**: never run one lane in `packages/shared/` while the other is in a
      consumer of it. A shared change plus an un-updated importer is exactly the breakage the
      workspace-dependency rule exists to prevent.
- [ ] **Allocation confirmed with the maintainer**: ADR numbers and the dependency-lane holder are
      maintainer-allocated. An open Dependabot PR is a queued proposal, not the lane holder.
- [ ] **Deploys serialized**: one merge at a time. Wait for the deploy *and* its health verify to
      finish before the next PR merges — overlapping deploys restart services and 502 the demo.
- [ ] **Generated-file churn handled**: `apps/landing/src/data/docs/` is git-tracked and
      regenerated by `npm test`. Revert timestamp/HEAD-sha churn before committing on both machines.
