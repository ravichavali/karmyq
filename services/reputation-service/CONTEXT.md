...
- **2026-08-22 (Sprint 126 review round — trust-score refresh, anomaly severity, carry lineage)**:
  ...
  - **`UNEXPECTED_KARMA_PROJECTION` now requires PROVEN lineage to be informational.** Fusion/fission legitimately carry canonical rows into a community replay never selects, but an arbitrary or corrupt canonical row influences the stored score identically. The preflight loads lineage from `communities.community_links` (`fusion_origin`, `split_origin`) and `communities.split_proposals` (parent → both children, since fission records only a sibling link), and blocks any canonical row with no lineage to the match's request communities.
