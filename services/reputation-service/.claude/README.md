# reputation-service - Local Context
...
cat ../../services/registry.json | jq '.services."reputation-service"'
...
# View impact report
cat services/impact-analysis.md | grep -A 10 "reputation-service"
```

### 3. Check Current Health
```bash
# Health check
curl -f http://localhost:3004/health || echo "Service is DOWN"

# View recent logs (if using pm2)
pm2 logs reputation-service --lines 50 --nostream
```
...
pm2 logs reputation-service --lines 20
```
...
### Service Won't Start
1. Check logs: `pm2 logs reputation-service`
2. Verify environment variables: `cat .env`
3. Test database connection: `psql $DATABASE_URL -c "SELECT 1"`
4. Check port availability: `lsof -i :3004`
