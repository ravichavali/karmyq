# notification-service - Local Context
...
cat ../../services/registry.json | jq '.services."notification-service"'
...
# View impact report
cat services/impact-analysis.md | grep -A 10 "notification-service"
```

### 3. Check Current Health
```bash
# Health check
curl -f http://localhost:3005/health || echo "Service is DOWN"

# View recent logs (if using pm2)
pm2 logs notification-service --lines 50 --nostream
```
...
### Service Won't Start
1. Check logs: `pm2 logs notification-service`
2. Verify environment variables: `cat .env`
3. Test database connection: `psql $DATABASE_URL -c "SELECT 1"`
4. Check port availability: `lsof -i :3005`
