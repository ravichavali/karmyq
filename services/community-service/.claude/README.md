# community-service - Local Context

> **⚠️ CRITICAL**: When working in this directory, follow these steps EXACTLY.
> This is a critical service - impacts 3 other service(s).
...
- request-service (critical)
- messaging-service (critical)
- social-graph-service (critical)


**⚠️ IMPACT**: If this service fails, 3 service(s) will be affected!
...
cat ../../services/registry.json | jq '.services."community-service"'
...
# View impact report
cat services/impact-analysis.md | grep -A 10 "community-service"
```

### 3. Check Current Health
```bash
# Health check
curl -f http://localhost:3002/health || echo "Service is DOWN"

# View recent logs (if using pm2)
pm2 logs community-service --lines 50 --nostream
```
...
pm2 logs community-service --lines 20
```
...
# Restart service
pm2 restart community-service

# Verify health
curl -f http://localhost:3002/health

# Monitor logs for errors
pm2 logs community-service --lines 50
```
...
### Service Won't Start
1. Check logs: `pm2 logs community-service`
2. Verify environment variables: `cat .env`
3. Test database connection: `psql $DATABASE_URL -c "SELECT 1"`
4. Check port availability: `lsof -i :3002`
...
**Service Owner**: core
**Criticality**: critical
**Impact Radius**: 3 dependent service(s)
