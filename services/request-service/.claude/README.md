...
cat ../../services/registry.json | jq '.services."request-service"'
...
# View impact report
cat services/impact-analysis.md | grep -A 10 "request-service"
```

### 3. Check Current Health
```bash
# Health check
curl -f http://localhost:3003/health || echo "Service is DOWN"

# View recent logs (if using pm2)
pm2 logs request-service --lines 50 --nostream
```
...
pm2 logs request-service --lines 20
```
...
# Restart service
pm2 restart request-service

# Verify health
curl -f http://localhost:3003/health

# Monitor logs for errors
pm2 logs request-service --lines 50
```
...
### Service Won't Start
1. Check logs: `pm2 logs request-service`
2. Verify environment variables: `cat .env`
3. Test database connection: `psql $DATABASE_URL -c "SELECT 1"`
4. Check port availability: `lsof -i :3003`
