...
export default function InvitationChain({ chain, currentUserId, className = '' }: InvitationChainProps) {
  if (!chain || chain.length === 0) {
    return (
      <div className={`bg-surface rounded-lg p-4 border border-border ${className}`}>
  ...
  return (
    <div className={`bg-gradient-to-r from-primary-light to-accent-light rounded-lg p-5 border border-primary-medium ${className}`}>
      {/* Header */}
      <div className="flex items-center mb-4">
        <svg className="w-5 h-5 text-primary mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
        </svg>
        <h3 className="text-lg font-semibold text-text">Invitation Chain</h3>
      </div>
        ...
        {chain.map((node, index) => {
          ...
          return (
            <div key={node.id} className="flex items-start">
              {/* Vertical Line */}
              {!isLast && (
                <div className="absolute ml-4 mt-10 h-8 w-0.5 bg-primary-medium" />
              )}
              ...
              <div className="flex items-center w-full">
                ...
                <div className="ml-3 flex-1">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className={`font-medium ${isCurrentUser ? 'text-primary-dark' : 'text-text'}`}>
                    ...
                    {/* Role Badge */}
                    {isCurrentUser && (
                      <span className="px-2 py-1 bg-primary-light text-primary-dark text-xs rounded-full font-medium">
                        You
                      </span>
                    )}
                    {isLast && !isCurrentUser && (
                      <span className="px-2 py-1 bg-accent-light text-accent-dark text-xs rounded-full font-medium">
                        Original Inviter
                      </span>
                    )}
                  </div>
      ...
      {chain.length > 1 && (
        <div className="mt-4 pt-4 border-t border-primary-medium">
          <p className="text-sm text-text-muted">
            <span className="font-medium text-primary-dark">{chain.length - 1}</span> degree
...
export function InvitationChainSkeleton({ className = '' }: { className?: string }) {
  return (
    <div className={`bg-surface rounded-lg p-5 border border-border animate-pulse ${className}`}>
