...
export const REQUEST_TYPES: RequestTypeOption[] = [
  {
    ...
    icon: '🤝',
    color: 'bg-karmyq-brown-50 border-karmyq-brown-200 hover:border-karmyq-brown-500',
    mostUsed: true
  },
  {
    value: 'ride',
    ...
    icon: '🚗',
    color: 'bg-primary-light border-primary-medium hover:border-primary'
  },
  {
    value: 'service',
    ...
    icon: '🔧',
    color: 'bg-success-light border-karmyq-teal-300 hover:border-karmyq-teal-500'
  },
  {
    value: 'event',
    label: 'Event Help',
    ...
    icon: '🎉',
    color: 'bg-accent-light border-accent hover:border-karmyq-orange-500'
  },
  {
    value: 'borrow',
    label: 'Borrow Item',
    ...
    icon: '📦',
    color: 'bg-karmyq-brown-50 border-karmyq-brown-300 hover:border-karmyq-brown-500'
  }
]
...
export default function RequestTypeSelector({ selectedType, onSelectType, showExamples = false }: RequestTypeSelectorProps) {
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-lg font-semibold text-text mb-2">What type of help do you need?</h2>
        <p className="text-sm text-text-muted">Choose the category that best fits your request</p>
      </div>
        ...
        <div className="bg-primary-light border border-primary-medium rounded-lg p-4">
          <div className="flex items-start space-x-2">
            ...
            <div className="flex-1">
              <h4 className="font-medium text-primary-dark mb-1">Selected: {REQUEST_TYPES.find(t => t.value === selectedType)?.label}</h4>
              <p className="text-sm text-primary-dark">Click another type to change, or continue below</p>
            </div>
          </div>
        </div>
      )}
